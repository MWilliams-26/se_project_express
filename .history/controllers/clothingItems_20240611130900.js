...
const { BAD_REQUEST_ERROR, INTERNAL_SERVER_ERROR } = require('../utils/errors');
  ...
  ClothingItem.findByIdAndUpdate(itemId, { $addToSet: { likes: userId } }, { new: true })
    ...
    .catch((err) => {
      ...
      if (err.name === 'CastError') {
        return res.status(BAD_REQUEST_ERROR).send({ message: er });
      }
      return res.status(INTERNAL_SERVER_ERROR).send({ message: "Error from likeItem"});
    })
}
